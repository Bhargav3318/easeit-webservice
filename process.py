...
def clean_response(text):
    """Cleans the API response by removing markdown and formatting properly."""
    cleaned_lines = []
    skip_phrases = [
        "Here's a detailed", "I’ve planned", "Let’s dive in", "Start with this"
    ]
    for line in text.split("\n"):
        line = line.replace("*", "").replace("#", "").strip()
        if not line or any(line.startswith(skip) for skip in skip_phrases):
            continue
        cleaned_lines.append(line)
    return cleaned_lines

def process_data(data):
    """Generates a structured travel itinerary using the Groq API and cleans the output."""
    try:
        user_prompt = (
            ...
            f"Budget: ${data['budget']} USD (total for all travelers for the entire trip). "
            f"Preferred travel mode: [Flight/Train/Bus] from {data['nearest_airport']}. "
            f"Destination preferences: {data['preferences']}."
            "[start here] example"
            "\n\n[name of the city of Departure] to [name of the city of Destination]"
            ...
            "\n [Link to book with all inputs]"
            "\n#### **Day 2 & Beyond:** [Repeat full plan for each day]"
            "\n#### **Return Journey [date]**"
            ...
            "\n**Final Total Cost: $[Amount]**(fully utilized budget)"

            "[end here]"
            "\n\n**Guidelines:**"
            "\n give accurate Total Estimated cost by caculating expenses not even $1 should be inaccurate(most imp guideline)"
        ...
        if response.status_code == 200:
            raw_text = response.json().get("choices", [{}])[0].get("message", {}).get("content", "No response generated")
            return clean_response(raw_text)
        else:
            return [f"API Error: {response.status_code} - {response.text}"]
    except Exception as e:
        logging.error(f"Exception in process_data: {e}")
        return [f"Error: {e}"]
